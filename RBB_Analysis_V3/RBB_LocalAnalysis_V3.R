...
if (OVERRIDE_LOG) { # ... (rest of log reading logic) ...
  ...
  tryCatch({ processed_files_list <- unique(readLines(processed_log_file, warn = FALSE)); processed_files_list <- processed_files_list[processed_files_list != ""]; message(paste("  Found", length(processed_files_list), "files previously processed.")) }, error = function(e) { warning(paste("Could not read processed files log:", processed_log_file, "\nError:", e$message, "\nProcessing all found files.")); processed_files_list <- character(0) })
...
for (input_csv_path in input_files) {
  ...
  file_handled_this_iter <- FALSE # Flag to track if handled (empty or processed)
  ...
  tryCatch({
    ...
    relative_path <- sub(paste0("^", gsub("\\\\", "/", normalized_root), "/?"), "", gsub("\\\\", "/", file_dir))
    ...
    if (nrow(raw_data) == 0) {
      ...
      # Set flag indicating handled, the 'else' block below will be skipped
      ...
      if (!all(required_cols %in% names(raw_data))) {
        ...
        stop(paste("Input file missing required columns:", paste(missing_cols, collapse=", "))) # stop() triggers error handler
      }
      
      # Extract Metadata
      rat_id <- NA; date_str <- NA; time_str <- NA; exp_id <- NA; cohort_id <- NA; date_obj <- as.Date(NA)
      # ... (Metadata extraction logic remains the same) ...
      base_name_pattern <- "^(RBB\\d{2})_(\\d{8}_\\d{6})$"; path_pattern <- ".*/(experiment_(\\d{2}))/(cohort_(\\d{2}))/.*"
      base_match <- stringr::str_match(base_output_name, base_name_pattern); path_match <- stringr::str_match(gsub("\\\\", "/", input_csv_path), path_pattern)
      if (!anyNA(base_match) && NCOL(base_match) >= 4 && !anyNA(path_match) && NCOL(path_match) >= 5) {
        rat_id <- base_match[1, 2] ; date_str <- base_match[1, 3] ; time_str <- base_match[1, 4]; exp_id <- path_match[1, 3] ; cohort_id <- path_match[1, 5]; date_obj <- lubridate::ymd(date_str, quiet = TRUE)
        message(paste("  Extracted Metadata: Exp=", exp_id, " Cohort=", cohort_id, " Rat=", rat_id, " Date=", date_str))
      } else { warning(paste("Could not parse Experiment/Cohort/Rat/Date from path/filename:", input_csv_path, " - Metadata will be skipped.")) }
      ...
      # ... (Wheel logic and saving remains the same) ...
      ...
      # ... (File 3, 4, 5, 6, 7, 8 logic and saving remains the same) ...
      # == File 3 ==
      ...
      # Log successful processing in processed_files.log
      if (!OVERRIDE_LOG) {
      ...
      if(!is.na(rat_id) && !is.na(exp_id) && !is.na(cohort_id) && !is.na(date_obj)) {
        # ... (Manifest metadata collection logic remains the same) ...
      ...
      # Set flag indicating successful handling
      file_handled_this_iter <- TRUE
    ...
    message(paste("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"))
    # file_handled_this_iter remains FALSE if an error occurs
  }) # End tryCatch wrapper
  
  # Increment the main counter if the file was handled (empty or processed)
...
# ... (Manifest update logic remains the same) ...
message("\n========================================================")
